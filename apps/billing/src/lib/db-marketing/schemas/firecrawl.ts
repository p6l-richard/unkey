...
export const firecrawlResponses = mysqlTable(
  "firecrawl_responses",
  {
    ...
    language: varchar("language", { length: 512 }),
    ogTitle: varchar("og_title", { length: 512 }),
    ogDescription: varchar("og_description", { length: 512 }),
    ogUrl: text("og_url"),
    ogImage: varchar("og_image", { length: 512 }),
    ogSiteName: varchar("og_site_name", { length: 512 }),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").onUpdateNow(),
    error: text("error"),
    inputTerm: varchar("input_term", { length: 512 }),
